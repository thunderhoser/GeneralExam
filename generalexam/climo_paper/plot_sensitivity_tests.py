"""Plots sensitivity to minimum front length and probability."""
...
MIN_PROBABILITIES = numpy.array([0.65, 0.5, 0.8])
...
def _run(top_prediction_dir_name, monte_carlo_max_fdr, output_dir_name):
    ...
    for j in range(num_min_lengths):
        for i in range(num_properties):
            ...
            this_title_string = '{0:s} with minimum length = {1:d} km'.format(
            ...
            this_output_file_name = (
                '{0:s}/{1:s}_min-length-km={2:d}.jpg'
            ).format(
                output_dir_name, property_names[i], MIN_LENGTHS_KM[j]
            )
    ...
    min_length_file_name = '{0:s}/sensitivity_min_length.jpg'.format(
        output_dir_name
    )

    print('Concatenating panels to: "{0:s}"...'.format(min_length_file_name))
    imagemagick_utils.concatenate_images(
        input_file_names=panel_file_names,
        output_file_name=min_length_file_name,
        num_panel_rows=num_min_lengths, num_panel_columns=num_properties
    )
    imagemagick_utils.resize_image(
        input_file_name=min_length_file_name,
        output_file_name=min_length_file_name,
    ...
    num_min_probs = len(MIN_PROBABILITIES)
    panel_file_names = []
    letter_label = None

    for j in range(num_min_probs):
        for i in range(num_properties):
            this_max_colour_value = (
                make_mc_figure.PROPERTY_TO_MAX_COLOUR_VALUE_DICT[
                    property_names[i]
            ...
            if j == 0:
                ...
                this_input_dir_name = (
                    '{0:s}/determinized/'
                    'prob-threshold={1:.3f}_min-length-metres=200000/'
                    'climatology'
                ).format(
                    top_prediction_dir_name, MIN_PROBABILITIES[j]
                )
            ...
            this_title_string = (
                '{0:s} with minimum probability = {1:.2f}'
            ).format(
                make_mc_figure.PROPERTY_ABBREV_TO_VERBOSE_DICT[
                    property_names[i]
                ],
            ...
            this_output_file_name = (
                '{0:s}/{1:s}_min-prob={2:.3f}.jpg'
            ).format(
                output_dir_name, property_names[i], MIN_PROBABILITIES[j]
            )
    ...
    min_prob_file_name = '{0:s}/sensitivity_min_probability.jpg'.format(
        output_dir_name
    )

    print('Concatenating panels to: "{0:s}"...'.format(min_prob_file_name))
    imagemagick_utils.concatenate_images(
        input_file_names=panel_file_names,
        output_file_name=min_prob_file_name,
        num_panel_rows=num_min_lengths, num_panel_columns=num_properties
    )
    imagemagick_utils.resize_image(
        input_file_name=min_prob_file_name,
        output_file_name=min_prob_file_name,
        output_size_pixels=CONCAT_FIGURE_SIZE_PX
    )
