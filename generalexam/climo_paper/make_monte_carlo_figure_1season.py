"""Makes 8-panel Monte Carlo figure with results for one season."""
...
VALID_COMPOSITE_NAMES_ABBREV = [
    'el_nino', 'strong_el_nino', 'la_nina', 'strong_la_nina'
]
...
SEASON_HELP_STRING = (
    'Season to plot.  Must be in the following list:\n{0:s}'
...
INPUT_ARG_PARSER.add_argument(
    '--' + COMPOSITES_ARG_NAME, type=str, nargs='+', required=False,
    default=VALID_COMPOSITE_NAMES_ABBREV, help=COMPOSITES_HELP_STRING
)
INPUT_ARG_PARSER.add_argument(
    '--' + SEASON_ARG_NAME, type=str, required=True, help=SEASON_HELP_STRING
)
INPUT_ARG_PARSER.add_argument(
...
def _run(top_input_dir_name, main_property_name, composite_names_abbrev,
         season_name, monte_carlo_max_fdr, output_file_name):
    """Makes 8-panel Monte Carlo figure with results for one season.
    ...
    if main_property_name == FREQUENCY_PROPERTY_NAME:
        ...
        property_names = [
            climo_utils.CF_AREA_PROPERTY_NAME, climo_utils.WF_AREA_PROPERTY_NAME
        ]
    ...
    for j in range(num_composites):
        for i in range(num_properties):
            ...
            this_input_dir_name = '{0:s}/{1:s}/{2:s}/stitched'.format(
                top_input_dir_name, composite_names_abbrev[j], season_name
            )
            ...
            _plot_one_difference(
                ...
                plot_latitudes=i == 0, plot_longitudes=j == num_composites - 1,
                plot_colour_bar=j == num_composites - 1,
    ...
    imagemagick_utils.concatenate_images(
        ...
        num_panel_rows=num_composites, num_panel_columns=num_properties
    )
...
if __name__ == '__main__':
    ...
    _run(
        ...
        season_name=getattr(INPUT_ARG_OBJECT, SEASON_ARG_NAME),
