...
def _run(actual_file_name, dummy_file_name, smoothing_radius_grid_cells,
    ...
    actual_saliency_matrix = (
        actual_saliency_dict[saliency_maps.SALIENCY_MATRIX_KEY]
    )
    ...
    dummy_saliency_matrix = (
        dummy_saliency_dict[saliency_maps.SALIENCY_MATRIX_KEY]
    )

    # Ensure that the two files contain the same examples.
    assert (
        actual_saliency_dict[saliency_maps.EXAMPLE_IDS_KEY] ==
        dummy_saliency_dict[saliency_maps.EXAMPLE_IDS_KEY]
    )
    ...
    if smoothing_radius_grid_cells is not None:
        ...
        for i in range(num_examples):
            for k in range(num_channels):
                actual_saliency_matrix[i, ..., k] = (
                    general_utils.apply_gaussian_filter(
                        input_matrix=actual_saliency_matrix[i, ..., k],
                        e_folding_radius_grid_cells=smoothing_radius_grid_cells
                    )
                )

                dummy_saliency_matrix[i, ..., k] = (
                    general_utils.apply_gaussian_filter(
                        input_matrix=dummy_saliency_matrix[i, ..., k],
                        e_folding_radius_grid_cells=smoothing_radius_grid_cells
                    )
                )
    ...
    for i in range(num_examples):
        for k in range(num_channels):
            this_flat_array = numpy.ravel(actual_saliency_matrix[i, ..., k])
            these_flat_ranks = (
                scipy.stats.rankdata(this_flat_array, method='average') /
                len(this_flat_array)
            )
            actual_saliency_matrix[i, ..., k] = numpy.reshape(
                these_flat_ranks, actual_saliency_matrix[i, ..., k].shape
            )

            this_flat_array = numpy.ravel(dummy_saliency_matrix[i, ..., k])
            these_flat_ranks = (
                scipy.stats.rankdata(this_flat_array, method='average') /
                len(this_flat_array)
            )
            dummy_saliency_matrix[i, ..., k] = numpy.reshape(
                these_flat_ranks, dummy_saliency_matrix[i, ..., k].shape
            )
