...
def _plot_one_statistic(
    ...
    basemap_dict = plot_gridded_stats._plot_basemap(statistic_matrix)
    ...
    if plot_colour_bar:
        colour_bar_object = plotting_utils.plot_colour_bar(
            axes_object_or_matrix=axes_object, data_matrix=this_matrix,
            colour_map_object=COLOUR_MAP_OBJECT,
...
def _run(statistic_dir_name, output_file_name):
    """Makes 8-panel figure with gridded front statistics.
    ...
    for i in range(num_seasons):
        ...
        this_num_labels_matrix = this_statistic_dict[
            climo_utils.NUM_WF_LABELS_KEY]
        this_length_matrix_km = (
            this_statistic_dict[climo_utils.MEAN_WF_LENGTHS_KEY] * METRES_TO_KM
        )
        this_length_matrix_km[
            this_num_labels_matrix < MASK_IF_NUM_LABELS_BELOW
        ] = numpy.nan

        this_title_string = 'Mean WF length (km) in {0:s}'.format(
            season_strings_verbose[i]
        )
        this_output_file_name = '{0:s}_wf_{1:s}.jpg'.format(
        ...
        this_length_matrix_km = (
            this_statistic_dict[climo_utils.MEAN_CF_LENGTHS_KEY] * METRES_TO_KM
        )
        this_length_matrix_km[
            this_num_labels_matrix < MASK_IF_NUM_LABELS_BELOW
        ] = numpy.nan

        this_title_string = this_title_string.replace('Mean WF', 'Mean CF')
        this_output_file_name = '{0:s}_cf_{1:s}.jpg'.format(
            extensionless_output_file_name, season_strings_abbrev[i]
        )
        panel_file_names.append(this_output_file_name)
