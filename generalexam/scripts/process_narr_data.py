...
def _run(
        first_time_string, last_time_string, input_field_name,
        pressure_level_mb, top_input_narr_dir_name, top_processed_dir_name):
    """For each time step, processes a single NARR field and writes to file.
    ...
    if input_field_name == processed_narr_io.U_WIND_EARTH_RELATIVE_NAME:
        input_field_name_other_component = (
            processed_narr_io.V_WIND_EARTH_RELATIVE_NAME
        )
        ...
        input_field_name_other_component = None

    input_field_name_grib1 = _field_name_gg_to_grib1(
        field_name=input_field_name, pressure_level_mb=pressure_level_mb)

    rotate_winds = input_field_name in WIND_FIELD_NAMES
    if rotate_winds:
        input_field_name_grib1_other_component = _field_name_gg_to_grib1(
            field_name=input_field_name_other_component,
            pressure_level_mb=pressure_level_mb)

        output_field_name = processed_narr_io.field_name_to_grid_relative(
            input_field_name)
        output_field_name_other_component = (
        ...
        input_field_name_grib1_other_component = None
        output_field_name = input_field_name + ''
        output_field_name_other_component = None
    ...
    for i in range(num_times):
        ...
        if valid_times_unix_sec[i] > LAST_GRIB_TIME_UNIX_SEC:
            ...
            this_netcdf_file_name = narr_netcdf_io.find_file(
                month_string=this_month_string, field_name=input_field_name,
            ...
            print 'Reading "{0:s}" at {1:d} mb from: "{2:s}"...'.format(
                input_field_name, pressure_level_mb, this_netcdf_file_name)
            field_matrix[0, ...] = narr_netcdf_io.read_data_from_file(
                this_netcdf_file_name, field_name=input_field_name,
                valid_time_unix_sec=valid_times_unix_sec[i],
                pressure_level_mb=pressure_level_mb)[0]

            if rotate_winds:
                print 'Reading "{0:s}" at {1:d} mb from: "{2:s}"...'.format(
                    input_field_name_other_component, pressure_level_mb,
                    this_netcdf_file_name_other_component)
            ...
            print 'Reading "{0:s}" at {1:d} mb from: "{2:s}"...'.format(
                input_field_name, pressure_level_mb, this_grib_file_name)
            ...
            if rotate_winds:
                ...
                field_matrix_other_component[
                    ...
                    grib_file_name=this_grib_file_name,
                    field_name_grib1=input_field_name_grib1_other_component,
                    model_name=nwp_model_utils.NARR_MODEL_NAME,
                    temporary_dir_name=None, wgrib_exe_name=WGRIB_EXE_NAME,
                    wgrib2_exe_name=WGRIB2_EXE_NAME, raise_error_if_fails=True)
        ...
        this_processed_file_name = processed_narr_io.find_file_for_one_time(
            ...
            field_name=output_field_name, pressure_level_mb=pressure_level_mb,
            valid_time_unix_sec=valid_times_unix_sec[i],
            raise_error_if_missing=False)

        print 'Writing "{0:s}" at {1:d} mb and {2:s} to: "{3:s}"...'.format(
            output_field_name, pressure_level_mb, valid_time_strings[i],
            this_processed_file_name)
        processed_narr_io.write_fields_to_file(
            pickle_file_name=this_processed_file_name,
            field_matrix=field_matrix, field_name=output_field_name,
        ...
        if rotate_winds:
            this_processed_file_name = processed_narr_io.find_file_for_one_time(
                top_directory_name=top_processed_dir_name,
                field_name=output_field_name_other_component,
                pressure_level_mb=pressure_level_mb,
                valid_time_unix_sec=valid_times_unix_sec[i],
                raise_error_if_missing=False)

            print 'Writing "{0:s}" at {1:d} mb and {2:s} to: "{3:s}"...'.format(
                output_field_name_other_component, pressure_level_mb,
